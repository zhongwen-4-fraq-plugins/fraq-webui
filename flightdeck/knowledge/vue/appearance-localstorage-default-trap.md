...
- 用户说"滑杆最低透明度"指滑杆**轨道本身的可见度下限**（20% → 25%，2026-08-12），不是可调值下限；曾误实现为 range min=20（导致滑杆调不到 0），最终方案是 JS 用组件颜色选择器预计算轨道色（透明度 ×60%、保底 25%）写入 `--app-slider-track-bg`，轨道颜色跟随组件且永不消失，其他组件不受影响。
- 新增界面卡片/输入框背景必须用 `var(--app-component-bg, var(--surface-2))`；直接写 `var(--surface-2)` 会脱离组件透明度设置（2026-08-12 插件配置弹窗的模型供应商卡片因此显示纯白，提交 69094ff 修复）。
