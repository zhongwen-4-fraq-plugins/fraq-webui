...
- 用户说"滑杆最低透明度 20%"指滑杆**轨道本身的可见度下限**，不是可调值下限；2026-08-12 曾误实现为 range min=20（导致滑杆调不到 0），最终方案是 JS 用组件颜色选择器预计算轨道色（透明度 ×60%、保底 20%）写入 `--app-slider-track-bg`，轨道颜色跟随组件且永不消失，其他组件不受影响。
