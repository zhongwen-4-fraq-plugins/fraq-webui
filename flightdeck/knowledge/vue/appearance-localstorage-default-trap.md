...
- 用户说"滑杆最低透明度 20%"指滑杆**轨道本身的可见度下限**，不是可调值下限；2026-08-12 曾误实现为 range min=20（导致滑杆调不到 0），正确做法是单个嵌套 color-mix 保底 20% 可见（详见 vue/multilayer-background-color-trap.md），组件透明度为 0 时轨道仍可见，其他组件不受影响。
